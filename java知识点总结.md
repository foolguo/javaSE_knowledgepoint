...
**封装性：将客观事物封装成类，并且该类只对信任的类或对象进行开放，对不信任的对象关闭，内部操作对外部不可见，注重保护性；**

**继承性：指一种特殊的能力，在不用编写任何代码的情况下就可以使用现有类的所有功能并且在不用重新编写的情况下对现有功能进行扩充；代码复用，**

**多态性：同一个方法或者对象在不同情况下的不同表现形式，（对象对态，方法多态）   （代码得到良好的设计）**
...
先产生类，再产生对象，类是产生对象的宏图（正向操作，后面有反向操作（反射：先得到类对象，再进行操作））
...
匿名内部类：和方法内部类一样，直接定义在方法中，但是没有名字，
接口名  接口对象 = new 接口名(){
		实现；
}
后面的lamda表达式有着异曲同工的效果 实现接口
```
...
1.简单工厂模式（应该用反射进行）：一个抽象产品类，多个具体产品类，一个具体工厂类，通过输入来确定制造那个产品（实例化那个对象）
...
可变参数：int...data   有且只有一个可变参数，如果参数列表中有多个参数 那么可变参数放到最后

静态倒入：  不推荐使用，因为不知道是哪个类调用的

for each

泛型：解决两个毫无关系的类发生向下转型时的 ClassCastException

通配符：解决使用泛型后参数统一化的问题；

<?>用在方法中，接收所有的泛型类型  **因为接收类型不确定，所以不可以通过set方法改变属性值**

<? extends T> 用在类和方法中，接收T及其T的子类，设置上限 **不可以通过set方法改变属性值**

<? super T> 用在方法中，接收T及其T的父类，设置下限  可以改，设置了上限 天然的向上转型
...
泛型接口：  继承泛型接口的子类可以选择继续是泛型类或者是规定类型的普通类

 枚举：

enum实际上是一种包装结构，它继承了一个Enum类，  枚举其实就是多例

枚举结构里可以有构造方法，普通方法，普通属性  但是  枚举对象一定要写在首行用  逗号  ，   隔开
...
1.注释：
   ...
   准确覆写         过期声明   Deprecated      压制警告

2.接口定义加强：可以有普通方法 和 静态方法 除了抽象方法和全局常量  原因 发现接口现有的功能不够用，如果通过抽象方法增加功能太危险

普通方法

```java
public default 返回值类型  方法名（）{}//这里的defalut不是访问权限控制符，就是在接口声明普通方法的标志
```
...
3.lamda表达式（函数式编程）（实际上就相当于匿名内部类）

1.规定lamda表达式实现接口，接口只可以有一个抽象方法

（）->{},

2.方法引用：（目的引用方法的实现，  应用方法的实现的功能和接口抽象方法要实现的功能一样  返回值 参数列表一样）
...
d.引用类的构造方法   类名::new    实际上 返回的是类的实例化对象  
...
反射：1.取得类对象，再根据对象实例化信息

​	    2.三种方式

​			a.通过Object类中的getClass()取得  ， 要根据类实例化对象取得类信息
